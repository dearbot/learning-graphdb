...
func main() {
	...
	go func ()  {
		...
		respBody, err := NumberQueryRequest(token, jsonString)
		if err != nil {
			fmt.Printf("error: %s\n", err)
		} else {
			sr:=SearchResponse{}
			...
			default:
				...
				for login, endCursor := range numberTasks {
					...
					g.Run(func() {
						...
						respBody, err := NumberQueryRequest(token, jsonString)
						if err != nil {
							fmt.Printf("error occurred in NumberQueryRequest: %s %s\n", login, err)
						} else {
							ur:=UserResponse{}
...
func NumberQueryRequest(token string, jsonStr []byte) (body []byte, err error) {
